...
function getContentType(res) {
  ...
  const urlMatch =
    url && new URL(url).href.match(/\.([a-zA-Z0-9]{1,8})($|\?|#)/);
...
function contentTypeByMimeString(contentType, ext) {
  if (
    contentType.includes(F_NTRIPLES) ||
    contentType.includes(F_NTRIPLES_DEP)
  ) {
    return F_NTRIPLES;
  } else if (
    contentType.includes(F_PLAIN) &&
    ['ntriples', 'nt'].indexOf(ext) >= 0
  ) {
    return F_NTRIPLES;
  } else if (
    contentType.includes(F_TURTLE) ||
    contentType.includes(F_TURTLE_DEP)
  ) {
    return F_TURTLE;
